...
enum VAD_Explanaton: String {
    ...
    \r● Since blood pressures are challenging to obtain, treat the mean arterial pressure. Reduce the cuff pressure until a constant sound is heard. This signifies the mean arterial pressure. Alternatively, in those who are admitted with an arterial line, place an ultrasound-guided arterial line.
    ...
    Critical Components: It's important to understand the critical components that make up present that make up the device[s]:
    \rBatteries:
    ...
    Overview:
    \rThe total artificial heart is a device used to bypass the cardiac system and replaces native heart function. It's indicated in patients with biventricular heart failure. It consists of two artificial pneumaticlly driven ventricles which are made from a biocompatible plastic. This prevents rejection from the body. The TAH is capable of producing a SV of 65 mL and CO of 12 L/min.
    \rWhat to know:
    ...
    \r● This pressure pushes blood in and out of the ventricles and creates produces a pusaltile flow that minics the natural cardiac rhythm- usually at fixed rate between 120-140 bpm.
    ...
    case HeartMateII = """
    Overview:
    \rThe HeartMate II is a second generation -continuous flow axial pump that can generate up to 10 L/min of flow at a pressure of 100 mmHg. It's synchronized with the activity of the heart, follows Frank-Starlings curve, and follows the native cardiac cycle.
    ...
    \r● ACLS protocols are able to be performed if necessary.
    ...
    \rWhat to know:
    \r● Perform CPR only when necessary- decrease the flow to P-2 (The P level is the speed which range from P-0: 10k RPM -P-9: 33K RPM).
    ...
    \r● Don't attempt to reposition the catheter. This should be done under flouroscopy.
