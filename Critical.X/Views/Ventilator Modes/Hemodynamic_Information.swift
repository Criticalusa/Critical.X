...
enum ParameterDetails: String {
    ...
    A SaO2 is determined by doing an Arterial Blood Gas (ABG). Values under 90% are considered low and require supplemental oxygen.
    ...
    \rThe results let us know whether cardiac output and oxygenation are adequate for the patient. If SV02 is decreased, then tissues are extracting a higher percentage of oxygen from the, and the CO is not high enough to meet normal 02 demands.
    ...
    \rDepending on the volume status, these pressures can change.  Changes in CVP does not directly reflect cardiac output.  Pressure changes should be jointly analyzed with cardiac output to determine the patient's fluid status.
    ...
    \rWWhen the RV contracts, there's trivial tricuspid regurgitation. This measured velocity across the tricuspid valve is equal to the RV pressure.
    ...
    """
    case CardiacOutput = """
    Cardiac output = stroke volume x heart rate.  This is the volume of blood to the heart pumps each minute.  CO predicts the delivery of oxygen to the cells and can vary at any time based on the body's metabolic needs.
    ...
    """
    case CardiacIndex = """
    \rCardiac index (CI) is CO / BSA. Not to get confused with cardiac output, CI assesses the cardiac output based solely on the patient's size.
    ...
    \rStroke volume variation is measured to test fluid responsiveness when CVP is not available or reliable. It measures the change is stroke volume after mechanical ventilation, which will either increase or decrease venous return. SVV is the difference in the amount of blood ejected from the left ventricle in the aorta for each beat. Usually, when we breathe the blood pressure decreases slightly during inspiration, and this is known as pulsus paradox.
    \rHowever, during intubation, positive pressure is exerted on the arteries which cause a reverse pulsus paradox, where the pressure is increased. The literature cites continuous SVV monitoring optimal for fluid responsiveness.
    """
    case SystemicVascularResistance = """
    SVR  is reflected when there are changes in the arterioles which affects left ventricular emptying.  SVR increases when there is peripheral vascular vasoconstriction- which ultimately increases stroke volume and cardiac output.
    ...
    \rUsually, increased SVR's will result in vasodilators such as night pride or diuretics if the preload is high.
    \rThose within low SVR may be treated with vasoactive agents such as norepinephrine, dopamine or fluids to treat hypotension.
    """
